...
import { useGetNewsByIdQuery, useGetNewsPublicQuery, useGetNewsPageBannerQuery } from "@/services/newsApi";
...
export default function NewsDetailPage() {
  ...
  const { data: bannerData } = useGetNewsPageBannerQuery();

  const newsItem = newsData?.data;
  const relatedNews = relatedNewsData?.data?.filter(n => n.id !== newsId).slice(0, 6) || [];
  const bannerImage = bannerData?.data?.background_image || 'https://static.vecteezy.com/system/resources/thumbnails/004/216/917/large/3d-world-news-background-loop-free-video.jpg';
  ...
  return (
    <div className="w-full">
      {/* Hero Section */}
      <PageHeroSection image={bannerImage} heading={'News'} alt={`${newsItem.title} Hero`} />
      ...
      <section className="w-full bg-white">
        ...
        >
          <div className="max-w-6xl pt-10 mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div
              className="space-y-6 text-gray-700 prose prose-lg "
